...
export function GuestDashboardClient() {
  ...
  async function switchTenant(tenantId:string){ chooseTenant(tenantId); await loadWorkspace(tenantId); }
  ...
  return <section className="mt-6 rounded-xl bg-white p-5 shadow-sm ring-1 ring-black/5">
    <div className="flex flex-wrap items-start justify-between gap-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button className="min-h-10 rounded-md bg-white px-3 text-sm font-semibold ring-1 ring-black/10" onClick={()=>void loadWorkspace(activeTenantId)} type="button">Yangilash</button>
      </div>
    </div>

    {message ? <p className="mt-4 rounded-md bg-amber-50 px-4 py-3 text-sm text-amber-800">{message}</p> : null}
    <div className="mt-5 grid gap-3 sm:grid-cols-4"><DashboardStat label="Saytlar" value={String(sites.length)} /><DashboardStat label="Published" value={String(publishedCount)} /><DashboardStat label="Views" value={String(totalViews)} /><DashboardStat label="QR scans" value={String(totalScans)} /></div>

    {sites.length === 0 ? <div className="mt-5 rounded-lg bg-slate-50 p-5 ring-1 ring-black/5"><p className="font-semibold">Bu workspace’da hali sayt yo‘q</p><p className="mt-2 text-sm leading-6 text-slate-600">Yangi V2 Site Studio orqali birinchi saytni yarating.</p><Link className="mt-4 inline-flex min-h-11 items-center rounded-md bg-teal-700 px-4 text-sm font-semibold text-white" href={`/guest/builder?plan=plus${activeTenantId ? `&tenant=${activeTenantId}` : ""}`}>Yangi sayt yaratish</Link></div> : <div className="mt-5 grid gap-4 lg:grid-cols-2">{sites.map((site)=><SiteCard analytics={analytics[site.id]} domains={domains.filter((domain)=>domain.site===site.id)} key={site.id} qrCodes={qrCodes.filter((qr)=>qr.site===site.id)} onChanged={()=>loadWorkspace(activeTenantId)} site={site}/>)}</div>}
  </section>;
}
