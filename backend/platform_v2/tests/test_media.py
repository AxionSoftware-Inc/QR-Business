...
class MediaAssetApiTests(APITestCase):
    ...
    def png_upload(self, name="cover.png", suffix=b""):
        return SimpleUploadedFile(name, self.png_bytes() + suffix, content_type="image/png")

    def test_valid_image_is_tenant_scoped_and_deduplicated(self):
        first = self.client.post("/api/v2/media/", {"tenant": str(self.tenant.id), "file": self.png_upload(), "alt": "Cover"}, format="multipart")
        ...
        second = self.client.post("/api/v2/media/", {"tenant": str(self.tenant.id), "file": self.png_upload("same.png")}, format="multipart")
        self.assertEqual(second.status_code, 200)
        self.assertEqual(second.data["id"], first.data["id"])
        self.assertEqual(MediaAsset.objects.count(), 1)

    def test_appended_payload_is_removed_by_canonical_reencode(self):
        marker = b"<script>polyglot-marker</script>"
        response = self.client.post("/api/v2/media/", {"tenant": str(self.tenant.id), "file": self.png_upload("polyglot.png", marker)}, format="multipart")
        ...
        self.assertNotIn(marker, stored_bytes)
        with Image.open(BytesIO(stored_bytes)) as image:
            self.assertEqual(image.size, (32, 24))

    def test_media_list_is_paginated(self):
        response = self.client.get(f"/api/v2/media/?tenant={self.tenant.id}")
    ...
    def test_fake_image_payload_is_rejected_even_with_image_mime(self):
        ...
        response = self.client.post("/api/v2/media/", {"tenant": str(self.tenant.id), "file": fake}, format="multipart")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(MediaAsset.objects.count(), 0)

    def test_user_cannot_upload_into_unowned_tenant(self):
        response = self.client.post("/api/v2/media/", {"tenant": str(self.other_tenant.id), "file": self.png_upload()}, format="multipart")
