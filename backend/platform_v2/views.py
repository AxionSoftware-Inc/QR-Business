...
class TenantViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return Tenant.objects.filter(id__in=user_tenant_ids(self.request.user)).distinct().order_by("name", "id")
    ...
    def entitlements(self, request, pk=None):
        return Response(entitlement_payload(self.get_object()))
    ...
    def get(self, request, code):
        qr = (
            ...
            .filter(
                ...
                site__tenant__status__in=ACTIVE_TENANT_STATUSES,
            )
            .first()
        )
        if not qr:
