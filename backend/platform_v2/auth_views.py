...
from .models import AuthSession, Identity, Membership
...
def _refresh_days():
    return int(os.getenv("JWT_REFRESH_DAYS", "30"))


def _cookie_name():
    return os.getenv("JWT_REFRESH_COOKIE_NAME", "qr_refresh")
...
def _new_session(user):
    ...
    session = AuthSession.objects.create(
        ...
        expires_at=timezone.now() + timedelta(days=_refresh_days()),
    )
...
def _set_session_cookie(response, raw_token):
    response.set_cookie(
        ...
        path="/api/v2/auth/",
        domain=os.getenv("JWT_REFRESH_COOKIE_DOMAIN") or None,
    )
...
def _resolve_google_user(claims):
    ...
    if not subject or not email:
        raise ValueError("Google credential is missing subject or email.")
    ...
    identity = (
        ...
        .select_related("user")
        .filter(provider=Identity.Provider.GOOGLE, subject=subject)
        .first()
    )
    if identity:
    ...
    family_name = str(claims.get("family_name") or "")[:150]
    changed = False
...
class SessionRefreshView(APIView):
    ...
    def post(self, request):
        ...
        if not session:
            response = Response({"detail": "Session expired or revoked."}, status=status.HTTP_401_UNAUTHORIZED)
